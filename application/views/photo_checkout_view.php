...
<body>
	...
	<div class="r_container">
		<div class="main rc-m-6">
			<h2>Checkout Info</h2>
			<h3>Total Photos: <?= $cartCount ?></h3>
			<h3>Photo Credits: <?= $photoCredits ?></h3>
			<h3>Your subtotal: $<?= $subtotal ?></h3>
			<h3><?= STATE_TITLE ?> sales tax: $<?= $state_sales_tax_total ?></h3>
			<h3>Checkout total: $<?= $overall_total ?></h3>
		...
		<div class="sidebar rc-m-6">
			...
			<form action="/process/buyphoto" method="POST" id="payment-form">
		...
		<p><h6><span class="login_page_field"><input type="hidden" id="total" name="total" value="<?= $overall_total ?>" /></span><label>Total:</label> $<?= $overall_total; ?></h6></p>	
		  ...
		  <div class="medium-6 column col-md-2">
   		...
   		<? } 
   			...
   			<p><h6><span class="login_page_field"><input type="hidden" id="total" name="total" value="<?= $overall_total ?>" /></span><label>Total:</label> $<?= $overall_total; ?></h6></p>
   			<p> <a href="/process/freephotos"><button class="btn btn-secondary button" type="submit">Check Out</button></a> </p>

   		<?	} ?>

			<p><a href="/photocart"><button class="btn btn-warning button warning">Cancel</button></a></p>
			<h5><a href="/terms">Terms</a> ˙ <a href="/privacy">Privacy Policy</a></h5>
		</div>
	</div>
	<div class="footer rc-m-12">
		<p><?= FOOTER_TEXT ?></p>
	</div>
</body>
</html>
