<!DOCTYPE html>
<html><!-- based on Ryukyu HTML5 starter index.html template 0.2.3 by Dan McKeown http://danmckeown.info -->
...
<body>
	<div class="container">
		...
		<div class="main row" id="user_files_list">
			<h2 class="text-center"><?= $title ?></h2>
			<section class='post_waffle small_waffle medium-5 columns'>
				<p><a href="/myphotos">Photos</a> you have selected: <?= $cartCount ?></p>
				<p>Your photo credits: <? echo $photoCredits; ?></p>
				<p>Your current subtotal: $<?= $subtotal ?></p>
				<p><a href="/photocheckout"><button class="button hollow">Check out</button></a>
			</section>
			<section class='post_waffle small_waffle medium-6 columns'>
				<h4>Photos in Cart</h4>
					<? 	
					$posts_latest_first = $posts;
					
						foreach ($posts_latest_first as $key => $value) {
							echo "<div class='myphotos myphotos_cart waffle waffle-photo-item col col-lg-3 columns medium-3'>";
					...
					//		var_dump($value);
							...
							{
							//	echo "<a href='/add/photocart/" . $value['post_id'] . "'><button disbaled class='disabled btn btn-info button hollow add_cart_button_' id='photo_add_cart_button'>add to cart</button></a>";
								echo "<a href='/remove/photocart/" . $value['post_id'] . "'><button class='btn btn-alert button alert hollow remove_cart_button_' id='photo_remove_cart_button'>&#x2716</button></a>";
							}
